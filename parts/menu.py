...
async def main(bot: Bot, db: DB, logger: Logger):
    ...
    bot.add_command(2, ['menu', 'get_started'], get_text_translations("menu_cmd_desc"))
    @bot.callback_query_handler(cs='get_started')
    @bot.message_handler(['get_started', 'menu'])
    @bot.message_handler(func=lambda msg: msg.text in get_text_translations('back_to_menu_btn').values())
    async def _get_started(obj: C|M):
        ...
        if not await db.is_rules_confirmed(obj.from_user.id):
            ...
            if c:
                await bot.edit(c.message, _('confirm_rules_text').format(rules_url=url), reply_markup=rm)
            elif m:
                await bot.reply(m, _('confirm_rules_text').format(rules_url=url), reply_markup=rm)
            else:
        ...
        rm = IM()
        rm.add(IB(_('copy_my_id_btn'), copy_text=CopyTextButton(obj.from_user.id)))
